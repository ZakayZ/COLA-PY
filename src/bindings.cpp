...
PYBIND11_MODULE(_cola_impl, mod) {
  ...
  py::class_<cola::Particle>(mod, "Particle")
      ...
      .def("__repr__", [](const cola::Particle& particle) -> std::string {
        auto ss = std::stringstream();
        ss << "Particle(pdg=" << particle.pdgCode << ", class=" << static_cast<int>(particle.pClass) << ")";
        return std::move(ss).str();
      });
  ...
  py::class_<cola::EventIniState>(mod, "EventInitialState")
      .def(py::init<int, int, double, double, double, float, float, int, int, int, int, int, int, int, float, float,
           ...
           "b"_a = 0.f, "n_coll"_a = 0, "n_coll_pp"_a = 0, "n_coll_pn"_a = 0, "n_coll_nn"_a = 0, "n_part"_a = 0,
           "n_part_a"_a = 0, "n_part_b"_a = 0, "phi_rot_a"_a = 0.f, "theta_rot_a"_a = 0.f, "phi_rot_b"_a = 0.f,
           "theta_rot_b"_a = 0.f, "ini_state_particles"_a = cola::EventParticles())
      .def_readwrite("pdg_code_a", &cola::EventIniState::pdgCodeA, "PDG code of the projectile")
      .def_readwrite("pdg_code_b", &cola::EventIniState::pdgCodeB, "PDG code of the target")
      .def_readwrite("pz_a", &cola::EventIniState::pZA, "Axial momentum of the projectile")
      .def_readwrite("pz_b", &cola::EventIniState::pZB, "Axial momentum of the target")
      .def_readwrite("energy", &cola::EventIniState::energy, "Incident energy of the event")
      .def_readwrite("sect_nn", &cola::EventIniState::sectNN, "Nucleon-Nucleon cross section from generator")
      .def_readwrite("b", &cola::EventIniState::b, "Impact parameter of the event")
      .def_readwrite("n_coll", &cola::EventIniState::nColl, "Total number of collisions")
      .def_readwrite("n_coll_pp", &cola::EventIniState::nCollPP, "Number of proton-proton collisions")
      .def_readwrite("n_coll_pn", &cola::EventIniState::nCollPN, "Number of proton-neutron collisions")
      .def_readwrite("n_coll_nn", &cola::EventIniState::nCollNN, "Number of neutron-neutron collisions")
      .def_readwrite("n_part", &cola::EventIniState::nPart, "Total number of participants")
      .def_readwrite("n_part_a", &cola::EventIniState::nPartA, "Number of participants from projectile nucleus")
      .def_readwrite("n_part_b", &cola::EventIniState::nPartB, "Number of participants from target nucleus")
      .def_readwrite("phi_rot_a", &cola::EventIniState::phiRotA, "Polar angle φ of rotation of projectile nucleon")
      .def_readwrite("theta_rot_a", &cola::EventIniState::thetaRotA, "Polar angle Θ of rotation of projectile nucleon")
      .def_readwrite("phi_rot_b", &cola::EventIniState::phiRotB, "Polar angle φ of rotation of target nucleon")
      .def_readwrite("theta_rot_b", &cola::EventIniState::thetaRotB, "Polar angle Θ of rotation of target nucleon")
  ...
  py::class_<cola::EventData>(mod, "EventData")
      .def(py::init<cola::EventIniState, cola::EventParticles>(), "state"_a = cola::EventIniState(),
